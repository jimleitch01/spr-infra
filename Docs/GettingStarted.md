# Test-Rig SelfService CICD / Getting Started
 ...
 * Health Checks
 * Kick Off Build from commit
 * Move to Azure GIT
 * FE-BE Build
 * Database Build and provisioning with Flyway
 * Put populator into build code
 * Bake build number into docker image
 * Random SoapUI shit (weather)
 * Just In Time Azure access
 * Add Sonar to Pipeline


# PetClinic SelfService CICD Description

The Test-Rig SelfService / CICD reference architecture provides a means of

[BLAH BLA SELF SERVICVE - BLAH BLAH CICD]

Petclinic demo uses the spring-boot-petclinic Java demo app

[https://github.com/spring-projects/spring-petclinic]()


There are multiple versions of this app. The version we are currently using is a **monolithic** spring boot framework java app with an in-memory database. Frontend and backend are all kept in the one single binary. The repo is called spr-monolith

The next cut of the system will split out the java REST backend and a React.js frontend. The database will be an external Postgres or MySQL database into which the deployer can choose a version of the data to be deployed. 
